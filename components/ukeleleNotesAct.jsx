...
export default function UkeleleNotesAct() {
  ...
  const start = () => {
    ...
    setNote(ukeleleNotes[randomId]);
    setOption([ukeleleNotes[randomIdAsw], ukeleleNotes[randomId], ukeleleNotes[randomIdAswt]].sort(function () { return Math.random() - 0.5 }));
    setPlaying(true);
  }
  const isItOk = () => {
        ...
        <div className={styles.courseGuitar__actFalse}>
          <div>
            ...
            <div className={styles.courseGuitar__actFalse_atc}>
              ...
              <Image unoptimized loader={({ src }) => src} src="https://res.cloudinary.com/atusdedos/image/upload/v1667276180/ukeleleExercises/ukeleleActN_ogivxo.png" alt="play" width={550} height={250} />
            </div>
          </div>
            ...
            {option.map((value, index) => {
              ...
              return (
                <button type="button" onClick={() => setAnswer(value.name)} className={value.name === answers ? styles.noteAct__options_active : null} key={keyIndex}>
                  <Image unoptimized loader={({ src }) => src} src={value.img} alt="play" width={298} height={101} />
